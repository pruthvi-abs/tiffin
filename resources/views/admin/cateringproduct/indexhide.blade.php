...
<div class="page-title">
      ...
      <div class="custom-list">
        <div class="btn-group">
          <label class="">
            <span>{!! Html::linkRoute('cateringproductall','All',array(),['class'=>'btn btn-primary ']) !!}</span>
          </label>
          <label class="">
            <span>{!! Html::linkRoute('cateringproduct.index','Visible',array(),['class'=>'btn btn-primary ']) !!}</span>
          </label>
          <label class="">
            <span>{!! Html::linkRoute('cateringproducthide','Hidden',array(),['class'=>'btn btn-primary active']) !!}</span>
          </label>
        </div>
      </div>
		</div>
    <div class="col-md-3 col-sm-3 form-group ">
         ...
         {!! Html::linkRoute('cateringproduct.create','Add New',[],['class'=>'btn-sm btn btn-primary margin']) !!}
      </div>
    </div>
</div>
...
$(document).ready(function(){
 oTable = $('#datatable').DataTable({
		"processing": true,
		"serverSide": true,
    "ajax": "{{ route('getCateringproducthide') }}",
    "aaSorting": [[0, 'asc']],
    "columns": [
  ...
  oTable.on('click','.visibilityclick', function(){
    ...
    /*
     swal({
      ...
      cancelButtonClass: "btn-sm btn btn-danger",
      cancelButtonText: "No",
      closeOnConfirm: true
    }, function (isConfirm) {
        if(isConfirm){
    */
          $.ajax({
            url:'{!! url('/cateringvisibility') !!}'+'/'+dataid,
            type:'POST',
      ...
      /*
          ...
          var founddata = $("div.visibility"+dataid).hasClass("active");
          if(founddata==true){
            $("div.visibility"+dataid).removeClass("active");
          }else{
            $("div.visibility"+dataid).addClass("active");
          }
        }
      });
    */
...
function renderaction(data, type, row){
		    url='{!! url('/cateringproduct') !!}'+'/'+data+'/edit';
        deleteurl='{!! url('/cateringproduct') !!}'+'/'+data;
