...
@section('content')
@php
  $theme_data = App\ThemeSetting::select('tenant_image','tenant_title','tenant_favicon','currency','cancel_reasons')->where('deleted_at',null)->where('id',1)->first();
@endphp
<style>
.btn-toggle {
  margin: 0 4rem;
          ...
          <div class="row">
					 <div class="col-md-6">
						 <div class="form-group">
							 ...
							 <input type="number" value="" step='0.01' min="0.1" id="payment_amount" name="payment_amount" class="form-control required"/>
      ...
      <div class="modal-body">
        ...
        <input type="hidden" value="Refund" id="r_payment_status" name="r_payment_status" class="form-control required"/>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="radio">
                 <label><input type="radio" value="Cash" checked name="r_payment_type" class="report_date form-control required"/> <span>Cash</span></label>
                 <label><input type="radio" value="Check" name="r_payment_type" class="report_date form-control required"/>  <span>Check</span></label>
                 <label><input type="radio" value="Credit Card" name="r_payment_type" class="report_date form-control required"/>  <span>Credit Card</span></label>
                 <label><input type="radio" value="Paypal"  name="r_payment_type" class="report_date form-control required"/>  <span>Paypal</span></label>
              </div>
          ...
          <div class="row">
					 <div class="col-md-6">
						 <div class="form-group">
							 ...
							 <input type="number" value="" step='0.01' min="0.1" id="r_payment_amount" name="r_payment_amount" class="form-control required"/>
      ...
      <script>
        $(document).ready(function($) {
          $("#refundpaymentform").validate({
            rules: {
              r_payment_details:  "required",
              r_payment_amount:  "required",
              r_payment_date:  "required",
            },
            messages: {
              r_payment_details:  "This field is required",
              r_payment_amount:  "This field is required",
              r_payment_date:  "This field is required",
            },
            submitHandler: function(form) {
              form.submit();
            }
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" value="" id="c_order_id" name="c_order_id" class="form-control required"/>
          <div class="row">
            <?php $reasons = explode(",",$theme_data->cancel_reasons); ?>
            <div class="col-md-12">
 						 <div class="form-group">
 							 {!! Form::label('cancel_reason','Reason') !!}
                <select class="form-control required cancel_reason" name="cancel_reason">
                  <option value="">Select Option</option>
                  <?php for($u=0;$u<count($reasons);$u++){ ?>
                    <option value="{{$reasons[$u]}}">{{$reasons[$u]}}</option>
                  <?php } ?>
                </select>
 							 @if ($errors->has('cancel_reason'))
 								 <p class='text-red'>{{ $errors->first('cancel_reason') }}</p>
 								@endif
 						 </div>
 						</div>
					 <div class="col-md-12">
               ...
               <textarea value="" id="c_notes" name="c_notes" class="c_notes form-control"></textarea>
      ...
      <script>
        $(document).ready(function($) {
          $("#cancelorderform").validate({
            ...
            messages: {
              cancel_reason:  "This field is required",
            },
            submitHandler: function(form) {
              form.submit();
            }
...
$(document).ready(function(){
 oTable = $('#datatable').DataTable({
    ...
    'createdRow': function( row, data, dataIndex ) {
      $(row).addClass( data['order_status'] );
    }
	});
	oTable.draw();
  ...
  oTable.on('click','.approvepayment', function(){
      ...
      $('#payment_amount').attr('max',data['remaining_total_amt'].substring(1));
      ...
      $('#editform').attr('action', '{!! url('/orderpaid') !!}'+"/"+s_oid[0]);
  ...
  oTable.on('click','.refundpayment', function(){
      ...
      $('#r_payment_amount').attr('max',data['remaining_total_amt'].substring(1));
      //$('#refundpaymentform').attr('action', 'https://www.neoinventions.com/dailytiffin/orderrefund/'+s_oid[0]);
      $('#refundpaymentform').attr('action', '{!! url('/orderrefund') !!}'+"/"+s_oid[0]);
...
function renderactionspickup(data, type, row){
  ...
  var payment_s=fdata[3];
  //if(dataorderstatus!="Paid"){
